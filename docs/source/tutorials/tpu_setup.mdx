...
1. Navigate to the TPU dashboard at: https://console.cloud.google.com/compute/tpus
   - Note: You'll need to enable the TPU API if you haven't already
   - A valid billing account must be linked to your project

2. If prompted, enable the TPU API for your project

![TPU Dashboard](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/optimum/tpu/gcp_tpu_dashboard.png)

## Step 2: Create your TPU Instance

Click the "Create" button to setup your TPU instance.

![TPU Setup](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/optimum/tpu/gcp_create_tpu_config.png)

### Region Selection

1. Review available regions and zones for TPUs: https://cloud.google.com/tpu/docs/regions-zones
2. For this example, we'll use `us-west-4a` zone
   - Important: TPU availability may vary by region
   - Tips: Choose a region closest to your primary usage location
...
2. Choose a runtime:
   ...
   - This runtime is optimized for TPU v5e
      - More runtime information on runtime can be found in at recommended runtime for TPU section in our [TPU hardware page](../conceptual_guides/tpu_hardware_support)
...
1. Access your TPU:
   ...
   ![TPU SSH](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/optimum/tpu/gcp_ssh_tpu.png)
...
2. For permanent SSH access:
   ...
   - You can also look at the ssh section in our [guide about the gcloud cli](../howto/gcloud_cli)
...
1. **Model Serving on TPU**
   - Follow our serving tutorial: [First Model Serving on TPU](../tutorials/inference_on_tpu)
   - Learn how to deploy and serve ML models efficiently on TPU

2. **Model Training on TPU**
   - Start with our training guide: [First Model Training on TPU](../tutorials/training_on_tpu)
