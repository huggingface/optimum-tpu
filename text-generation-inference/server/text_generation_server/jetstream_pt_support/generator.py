...
from jetstream.engine.tokenizer_api import Tokenizer
...
class Slot:
    ...
    def __init__(self, id: int, tokenizer: Tokenizer):
        self._id = id
...
class TpuGeneratorJetStream(Generator):
    ...
    def __init__(
        ...
        self.tokenizer_js = engine.build_tokenizer(metadata)
    ...
    def prefill(self, batch: Batch) -> Tuple[List[Generation], CachedBatch]:
        ...
        for request in batch.requests:
            ...
            slot = Slot(self._get_slot_id(), self.tokenizer_js)
            self.slot_index += 1
        ...
        for slot in self.slots:
            # Tokenize the inputs
            tokens, true_lengths = self.tokenizer_js.tokenize(slot.inputs)
            ...
            self.decode_state = self.engine.insert(prefill_results, self.decode_state)
    ...
    @classmethod
    def from_pretrained(cls, model_path: str, max_batch_size: int, max_sequence_length: int):
        """Instantiate a TpuGenerator that uses JetStream/Pytorch engine.
