...
def test_info(model_path):
    ...
    # For now jetstream returns "meta" for all models, even if it runs on TPU
...
def test_decode_multiple(model_path):
    ...
    import os
    if os.environ.get("JETSTREAM_PT", "0") == "0":
        # from text_generation_server.generator import TpuGeneratorSingleThread as AutoGenerator
        from text_generation_server.generator import TpuGenerator as AutoGenerator
    else:
        from text_generation_server.auto_generator import AutoGenerator
    ...
    for _ in tqdm(range(gen_tokens - 1), "Decoding tokens (finishing)"):
        ...
        g = generations[0]
        tokens[g.request_id].append(g.tokens.ids[0])
        assert g.request_id == 1
        print(f"@1: {tokens[1][-1]}")
    assert next_batch is None
    output = generations[0].generated_text
    assert output.generated_tokens == max_new_tokens
    print(f"tokens[0]: {tokens[0]}")
    print(f"tokens[1]: {tokens[1]}")
    tokenizer = generator.tokenizer
    print(f"tokens[0]: {tokenizer.decode(tokens[0])}")
    print(f"tokens[1]: {tokenizer.decode(tokens[1])}")
